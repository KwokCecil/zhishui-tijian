...
- 体检得分 = 100 - 命中加权分（高3/中2/低1），只是参考量；
- 风险等级按命中特征的**严重程度**直接判定：
  - 高风险：≥2 条高危特征；或 高危+中危 同时命中；或触发 R17+R19+R35 关键预警组合；
  - 中风险：1 条高危特征；或 ≥2 条中危特征；或 中危+提示；
  - 低风险：其余。
- 所以案例一（小微临界预警链）、案例六（收购发票身份存疑）即使健康分95，等级也是高风险。
