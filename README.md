...
- 规则层级：C01 是 R17（所得临界）+ R39（大额调减）的组合规则，
  组合规则用独立 C 系列编号、单独执行，同时命中时 R17/R39 并入 C01 不重复计分；
  R19 编号已腾空，原子规则编号保持稳定。
- 规则分类：按主题分组编号（发票/资金/上下游/申报/人资/案例启发/加油站/团伙/资格/数据/农产品），
  执行与展示按分类排序；组合规则独立编号（C01 起），评分层的组合加成是另一层概念。
- 政策互斥：P01 小微低税率与 P02 高新15% 互斥，同一所得择优适用，小微 5% 更优。
- 风险-政策联动：P01/P08↔R17/C01/R35、P02↔R30、P03↔R20，命中相关风险时提示“享受前提”，
  所有场景通用，不针对单个案例。
- 六税两费适用对象：小规模纳税人、个体工商户、小型微利企业；P04 负面清单为限制规则，
  命中=不得享受，未触发=不影响。
- 例：案例一命中 C01(20)+R35(20)=40 基础分（R17/R39 已并入 C01），组合 +25 → 65/100 → 高风险；
