...
- 规则层级：R19 是 R17（所得临界）+ R39（大额调减）的复合规则，
  同时命中时 R17/R39 并入 R19 不重复计分。
- 规则分类：按主题分组编号（发票/资金/上下游/申报/人资/案例启发/加油站/团伙/资格/数据/农产品），
  执行与展示按分类排序；R19 是组合规则，评分层的组合加成是另一层概念。
- 政策互斥：P01 小微低税率与 P02 高新15% 互斥，同一所得择优适用，小微 5% 更优。
- 风险-政策联动：P01/P08↔R17/R19/R35、P02↔R30、P03↔R20，命中相关风险时提示“享受前提”，
  所有场景通用，不针对单个案例。
