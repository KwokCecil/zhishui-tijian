...
- 规则分类：R + 三位编码 = 第一位业务大类 + 后两位序号（R=Rule）
  ...
  只登记已实现规则，新增按类顺延；组合规则独立编号（G001 起），评分层的组合加成是另一层概念。
...
- 六税两费适用对象：小规模纳税人、个体工商户、小型微利企业；P104 负面清单为限制规则，
  命中=不得享受，未触发=不影响。
- 例：案例一命中 G001(20)+R604(20)=40 基础分（R601/R602 已并入 G001），组合 +25 → 65/100 → 高风险；
  案例六命中 R804(45)+R805(20)=65 → 高风险；2 条高危 = 90 → 高风险。
