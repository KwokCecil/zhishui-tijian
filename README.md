...
- 点数：高危 60、中危 20、低危/提示 5。
- 组合升级由规则层完成：G001 临界点聚集、G002 小微临界预警链，评分层无额外加成。
- 等级由指数直接推导：≥60 高风险；20-59 中风险；<20 低风险。
- 规则层级：G001 是 R601（所得临界）+ R602（大额调减）的组合规则，
  组合规则用 G 系列独立编号、单独执行，同时命中时 R601/R602 并入 G001 不重复计分。
...
- 六税两费适用对象：小规模纳税人、个体工商户、小型微利企业；P104 负面清单为限制规则，
  命中=不得享受，未触发=不影响。
- 例：案例一命中 G002(60)，R601/R602 并入 G001、G001/R604 并入 G002 → 60/100 → 高风险；
  案例六命中 R804(60)+R805(20)=80 → 高风险；2 条高危 = 120 封顶 100 → 高风险。
