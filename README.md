...
- 预警原则：一条高危或两条中危即触发高风险；G001 临界调减联动为中危，与 R604 叠加即预警。
- 分级标准：低危=非常常见、单看不能下结论；中危=很可能异常但可合理解释；高危=几乎可断定违法或直接证据。
- 等级由指数直接推导：≥60 高风险；30-59 中风险；<30 低风险。
- 规则层级：G001 是 R601（所得临界）+ R602（大额调减）的组合规则，
  组合规则用 G 系列独立编号、单独执行；G001-G005 已实现，命中时构成规则并入组合结果不重复计分。
- 规则分类：R + 三位编码 = 第一位业务大类 + 后两位序号（R=Rule）
  （发票/资金/上下游/申报/人资信用/资格优惠/数据质量/行业模板/关联团伙），
  只登记已实现规则，新增按类顺延；组合规则独立编号（G001 起），升级由规则层完成。
- 政策互斥：P101 小微低税率与 P102 高新15% 互斥，同一所得择优适用，小微 5% 更优。
- 风险-政策联动：P101/P301↔R601/G001/R604、P102↔R605、P103↔R603，命中相关风险时提示“享受前提”，
  所有场景通用，不针对单个案例。
