...
组合加成仅一条：命中小微临界家族（601 或 001）+ 604 +25（两条低/中危也能构成严重风险）。
...
def _bonuses(hits):
    """组合加成：小微临界家族（601 或 001）+ 收入利润不匹配（604）。
    组合规则按“家族+关联特征”判定，不重复计算已并入的原子规则。"""
    ids = {h.get("rule_id") for h in hits}
    small_micro = (ids & {"601"}) or ("001" in ids)
    if small_micro and "604" in ids:
        return [("小微临界×收入利润不匹配组合（601/001 + 604）", 25)]
    return []
