...
def check_r09(invoices):
    ...
    return _rule(
        ...
        "A级信用纳税人可在10个工作日内申请核实，核实通过可不转出（案例七）。",
    )
...
def check_r17(profile):
    ...
    if near:
        return _rule(
            ...
            "若符合小型微利企业条件，可叠加六税两费减半等优惠（案例一：风险处置+合规降负双轨）。",
        )
...
def check_r20(profile):
    ...
    if ratio > 15:
        return _rule(
            ...
            "研发费用畸高需核查研发真实性：立项、人员、工时、材料领用、辅助账（案例一）。",
        )
...
def check_r21(profile):
    ...
    if abs(diff) > 50:
        ...
        return _rule(
            ...
            "横向比对同地段/车流条件站点；偏离过大需核实是否存在少申报（案例三：多源交叉验证）。",
        )
...
def check_r24(profile):
    ...
    if count >= 5:
        return _rule(
            ...
            "单户看都合法、跨户汇总才暴露拆分：按关联户/关联人组织核查（案例四）。",
        )
...
def check_r27(invoices):
    ...
    if total and service / total > 0.8:
        return _rule(
            ...
            "服务费/咨询费虚开只能靠实质资料验证：人员考勤、服务记录、交付物、合同、付款凭证、物流（案例四核查清单）。",
        )
...
def check_r30(profile):
    ...
    if htech == "否" and deduction == "是":
        return _rule(
            ...
            "资格认定权（科技部门）与优惠享受权（税务）分离：输出高企资格复核清单（立项记录、高新收入占比、研发人员占比、知识产权、适用名单）；按提示提醒→约谈劝退→停止享受→提请复核→追缴处理（案例五）。",
        )
...
def check_r32(profile):
    ...
    if ratio > 30:
        ...
        return _rule(
            ...
            "无票采购（尤其私人炼油厂）导致'以进控销'的进端不完整；生产环境需罐车/油罐硬件计量与多源数据补位（案例三边界）。",
        )
...
def check_r33(profile):
    ...
    if doc == "是" and missing == "是":
        return _rule(
            ...
            "外部单据是戳穿数据删改的锚点：以进油单反推入库量，定性先行、定量后算（案例三）。",
        )
...
def check_r34(profile):
    ...
    if chip == "旧标准":
        return _rule(
            ...
            "旧芯片存在硬件漏洞（公安部2024-09典型案例、央视2025-01曝光）；新国标GB/T 9081-2023防篡改但未铺开——关键结论用外部数据交叉验证（案例三设备信任梯度）。",
        )
...
def check_r35(profile):
    ...
    if revenue > 5000 and ratio < 3:
        ...
        return _rule(
            ...
            "收入规模大但利润极薄：核查成本归集、研发加计、税前扣除是否合规；准备备查资料（案例一：292.7万临界+研发加计600万预警链）。",
        )
...
def check_r36(invoices):
    ...
    if not suspects.empty:
        ...
        return _rule(
            ...
            "纠正路径：可要求贩子到办税服务大厅代开增值税发票补正（案例六结局）。",
        )
...
def check_r37(invoices):
    ...
    if not big.empty:
        ...
        return _rule(
            ...
            "收购大户核实生产能力与自产证据：单户金额巨大且身份存疑时风险升级（案例六：11户开票1.27亿）。",
        )
...
def check_r38(invoices, fund_flows):
    ...
    if mismatches:
        return _rule(
            ...
            "发票流写企业←农户，资金流却对应贩子：存在中间商，应取得贩子开具的增值税发票（可要求到办税大厅代开补正）——三流不一致的农产品版（案例六结局）。",
        )
